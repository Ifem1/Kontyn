...
export function KontynShell({ route = "Mission" }: { route?: string }) {
  ...
  function loadDemo() { const source = "https://raw.githubusercontent.com/Ifem1/Kontyn/main/evidence/studionet-positive-lifecycle.txt"; const hash = "a".repeat(64); setOrg(""); setEpochNo(""); setActionId(""); setCapabilityId(""); setOrgName("Kontyn demonstration organization"); setCharter(JSON.stringify({ mission: "Demonstrate bounded treasury governance", epoch_duration_seconds: 300, source_bindings: [{ source_url: source, metadata_url: source, license_url: source, source_hash: hash, metadata_hash: hash, license_hash: hash, version_hash: hash }] }, null, 2)); setPolicy(JSON.stringify({ reserve_floor_wei: "10", max_spend_epoch_wei: "100" }, null, 2)); setCapability(JSON.stringify({ id: "demo-grant", action_type: "PAY_GRANT_RECIPIENT", risk_tier: "TIER_1", max_amount_wei: "10", beneficiary: "0x1111111111111111111111111111111111111111", challenge_duration_seconds: 300, allocation_expiry_seconds: 600 }, null, 2)); setManifest(JSON.stringify({ sources: [source] }, null, 2)); setFundWei("10"); setCounterSourceUrl(source); setCounterUrl(source); setCounterHash(hash); setNotice("Demo inputs loaded for form exploration only. No organization, epoch, action, or capability ID has been created or selected."); }
